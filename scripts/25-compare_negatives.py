...
if __name__ == "__main__":
    ...
    if model_type == "nvembedv2":
        ...
        neg_file = "/data/datasets/beir/msmarco/XC/raw_data/label.raw.csv" if new_negatives else "/data/datasets/beir/msmarco/XC/raw_data/ce-scores.raw.txt" 
        ...
        data_dir = (
            "/data/suchith/outputs/maggi/00_nvembed-to-compute-msmarco-embeddings-001/predictions/msmarco" 
            if new_negatives else "/data/outputs/mogicX/54_nvembed-for-msmarco-001/matrices/msmarco"
        )

        qry_neg_file = f"{data_dir}/negatives_trn_X_Y_normalize_thresh-50.npz" 
        qry_th_neg_1 = load_negatives(qry_neg_file, k=5) 

        qry_neg_file = f"{data_dir}/negatives_trn_X_Y_normalize_thresh-70.npz" 
        qry_th_neg_2 = load_negatives(qry_neg_file, k=5) 

        qry_neg_file = f"{data_dir}/negatives_trn_X_Y_normalize_thresh-90.npz" 
        qry_th_neg_3 = load_negatives(qry_neg_file, k=5) 

        qry_neg_file = f"{data_dir}/negatives_trn_X_Y_normalize_thresh-95.npz" 
        ...
        for i in tqdm(np.random.permutation(len(qry_ids))[:10_000]):
            ...
            if not new_negatives:
                examples[-1].update({"original negatives": [(neg_txt[p], float(q)) for p,q in zip(indices_1, scores_1)]})

        fname = "/home/sasokan/suchith/outputs/examples/06-msmarco_nv-embed-v2_negatives.json" if new_negatives else "/home/sasokan/suchith/outputs/examples/04-msmarco_negatives.json"
        ...
        data_dir = "/data/datasets/beir/msmarco/XC/cross_encoder" 

        qry_neg_file = f"{data_dir}/ce-negatives-topk-05_trn_X_Y_thresh-80.npz"
        qry_th_neg_1 = load_negatives(qry_neg_file, k=5) 

        qry_neg_file = f"{data_dir}/ce-negatives-topk-05_trn_X_Y_thresh-85.npz"
        qry_th_neg_2 = load_negatives(qry_neg_file, k=5) 

        qry_neg_file = f"{data_dir}/ce-negatives-topk-05_trn_X_Y_thresh-90.npz"
        qry_th_neg_3 = load_negatives(qry_neg_file, k=5) 
        ...
        for i in tqdm(np.random.permutation(len(qry_ids))[:10_000]):
            ...
            examples.append({
                ...
                "thresh 80 negatives": [(neg_txt[p], float(q)) for p,q in zip(indices_2, scores_2)],
                "thresh 85 negatives": [(neg_txt[p], float(q)) for p,q in zip(indices_3, scores_3)],
                "thresh 90 negatives": [(neg_txt[p], float(q)) for p,q in zip(indices_4, scores_4)],
            })

        fname = "/home/sasokan/suchith/outputs/examples/08-msmarco_ce-negatives.json"
        with open(fname, "w") as file:
            json.dump(examples, file, indent=4)
