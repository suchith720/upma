...
def upma_beir_inference(output_dir:str, input_args:argparse.ArgumentParser, mname:str, meta_save_fname:str, 
                        ...
                        exclude_module_from_tying:Optional[str]=None, device:Optional[str]="cuda", no_memory:Optional[bool]=False, 
                        n_layers:Optional[int]=6, real_world:Optional[bool]=False):
    ...
    for dataset in tqdm(datasets):
        ...
        output = upma_run(output_dir, input_args, mname, test_dset, train_dset, 
...
def upma_run(output_dir:str, input_args:argparse.ArgumentParser, mname:str, test_dset:Union[XCDataset, SXCDataset],
             ...
             resume_from_checkpoint:Optional[bool]=None, device:Optional[str]="cuda", no_memory:Optional[bool]=False, 
             n_layers:Optional[int]=6, real_world:Optional[bool]=False):
    ...
    def get_time(batch):
        batch = {k: v.to(device) for k, v in batch.items() if "lbl2data" not in k}
    ...
    number_of_batch = 1000 if device == "cuda" else 10
    ignore_batches = 100 if device == "cuda" else 4
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            if i < ignore_batches:
                t = get_time(batch)
            if i == number_of_batch + ignore_batches:
                break
            else:
                t = get_time(batch)
                t = t / batch["data_input_ids"].shape[0] * 1000 

                latencies.append(t)
                seq_len.append(batch["data_input_ids"].shape[1])

    return {
        "time_mean": np.mean(latencies), 
        "time_p95": np.percentile(latencies, 95), 
        "time_p50": np.percentile(latencies, 50),
        "seq_mean": np.mean(seq_len), 
        "seq_p95": np.percentile(seq_len, 95), 
        "seq_p50": np.percentile(seq_len, 50),
    }
...
if __name__ == '__main__':
    ...
    if real_world:
        file_tag = "cuda_fp16_real-world" if device == "cuda" else "cpu_fp32_real-world"
    else:
    ...
    if extra_args.case == 1:
        ...
        save_file = f"/home/sasokan/suchith/outputs/timing/07_msmarco-gpt-intent-substring-linker-with-ngame-loss-002_{file_tag}_no-memory_nlayers-06.json"
        ...
        save_file = f"/home/sasokan/suchith/outputs/timing/07_msmarco-gpt-intent-substring-linker-with-ngame-loss-002_{file_tag}_memory_nlayers-06.json"
        ...
        no_memory = update_config_during_inference = True
        save_file = f"/home/sasokan/suchith/outputs/timing/07_msmarco-gpt-intent-substring-linker-with-ngame-loss-002_{file_tag}_no-memory_nlayers-12.json"
    ...
    metrics = upma_beir_inference(output_dir, input_args, mname, "msmarco-intent-substring-conflation-01", meta_file, linker_dir, data_repr_pooling=False, 
                                  ...
                                  update_config_during_inference=update_config_during_inference, n_layers=n_layers, real_world=real_world)
