...
def main(do_inference):
    output_dir = "/home/aiscuser/scratch1/outputs/upma/18_cross-encoder-training-ms-marco-lambda-hard-neg-005"
    ...
    mname = f"{output_dir}/{sorted(os.listdir(output_dir))[-1]}/final"
    ...
    config_key = "data_lbl_gpt-all-intent-substring_ce-negatives-topk-05_exact"
...
def inference():
    output_dir = "/data/outputs/upma/18_cross-encoder-training-ms-marco-lambda-hard-neg-005"
    ...
    pos_pred = get_positive_inference(model, config["train"])
    neg_pred = get_negative_inference(model, config["train"])

    pred_dir = f"{output_dir}/predictions"
    os.makedirs(pred_dir, exist_ok=True)

    sp.save_npz(f"{pred_dir}/msmarco_trn_lbl.npz", pos_pred)
    sp.save_npz(f"{pred_dir}/msmarco_trn_neg.npz", neg_pred)


if __name__ == "__main__":
    # main(do_inference=True)
